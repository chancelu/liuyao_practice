...
export function buildBaziContext(c: BaZiChart, focus?: string): string {
  ...
  const past = withTrig.filter((y) => y.past).slice(-8);
  const future = withTrig.filter((y) => !y.past).slice(0, 8);
...
export function buildBaziReadingPrompt(): string {
  return [
    ...
    '【过去回顾】从给出的「过去有引动的流年」中挑 5-8 个最有标志性的年份，逐年一条、每条独立成行，格式为「年份 干支——触发机制——据盘理推测易发的事类」；触发机制要点明（如冲动夫妻宫、填实空亡、岁运并临、天克地冲、凑齐三合三刑），事类要具体（如升学变动、婚恋动象、职场压力、破财、远行、健康预警）；语气统一为「据盘理推测」，并提醒学员逐条对照验证——这是检验命盘判读是否正确的环节；',
    ...
    '【事业】看官杀状态、格局类型与用神五行：格局定行业类型，用神定方位与行业，官杀运定升迁期；结合大运序列指出适合发力与宜守成的阶段，给出具体的行业方向建议；',
    '【财运】看财星的强弱、透藏、是否空亡以及求财结构：食伤生财=技艺口才变现，财空=待岁运填实，身旺财弱=财运年发力、身弱财旺=财多反累；指出适合的求财方式与破财风险提示；',
    '【感情婚姻】男命看财星、女命看官星，兼看日支夫妻宫的合冲刑伏吟：夫妻宫逢合逢冲之年为婚恋应期，伏吟主矛盾反复；分析择偶倾向、婚姻稳定度与相处建议；',
    '【健康】看五行偏枯处与刑冲部位：过旺过弱的五行对应脏腑（木-肝胆、火-心与小肠血管、土-脾胃、金-肺与呼吸道、水-肾与泌尿生殖），刑冲之年容易应验；给出日常养护方向，并提醒身体不适以正规医疗为准；',
    '【子女】看时柱子女宫与子女星（男命看官杀、女命看食伤）的旺衰透藏与喜忌：判断子女缘分深浅、子女发展的大意，以及哪些大运流年利子女；',
    '【建议与趋避】从五行喜用角度给具体建议：适合的行业方向、方位、颜色、人际合作属相等；',
    '【给小白的话】两三句大白话总结，并提醒：八字是传统术数的趋势参考，「命好不如运好，运好不如心态好」，具体人生选择以现实努力与专业意见为准。',
    '要求：总长度 2000 字以内；通俗但专业；一切判断基于给出的命盘数据，不得编造干支与年份；注明典籍出处；若学员在「想了解的方面」中指定了专题，对应分节要写得最详细，其余分节保持精炼但不可省略。',
  ].join('\n');
}
