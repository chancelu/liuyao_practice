...
// ============ 格局 → 职业象对照（实战派速查，非经典原文） ============
// 用法：格局定「行业类型」，用神五行定「具体方位与行当」，二者叠加取象。
export const GEJU_CAREER: Record<string, string> = {
  正官格: '名分型职业：公务员、事业单位、大企业职能岗、行政管理、司法法务——一切「在名分之内」的平台。官印相生者尤宜体制内文教；财官双美者宜金融管理。',
  七杀格: '压力密集型权威：军警法检、外科医生、竞技体育、工程攻坚、安全风控、审计监察。杀印相生主技术权威；食神制杀主以技艺驭压力（主刀医生、王牌律师、总工）。',
  正财格: '稳健实业型：财务、会计、实业经营、门店、供应链、固定资产——看得见的进项。食伤生财者宜「技艺+经营」（手艺开店）；财官双美者宜企业中层至高管。',
  偏财格: '流动性财源：经商贸易、投资理财、销售商务、中介经纪、流量与平台生意、跨境业务。偏财透干有官护者，宜做资源整合与人脉变现。',
  正印格: '庇护与输入型平台：教育、学术科研、文化出版、公务文职、人力资源、心理咨询。官印相生走仕途文教；印重用财破者，反而宜从商管钱。',
  偏印格: '偏门专精：玄学命理、中医、艺术、冷门技术、侦探调查、自由研究、幕后策划。枭神得偏财制者，偏门可以做大；夺食者宜纯技术不宜经营。',
  食神格: '温和输出变现：餐饮食品、文娱内容、技艺创作、教育培训、儿童相关、生活方式行业。食神制杀者可以技艺入高压行业（名厨、名医、名家）。',
  伤官格: '锋芒输出型：创意策划、演艺表达、写作自媒体、演讲培训、律师辩护、产品设计。伤官生财者宜内容创业；伤官配印者宜学术创新与技术发明。',
  建禄格: '自创型立身：自由职业、个体经营、专业技术岗——不靠祖荫、自成家业。禄格财官透者宜「专业+经营」双轨（开工作室、技术入股）。',
  阳刃格: '刚性行业：武职军警、外科手术、竞技对抗、机械重工、消防应急。刃有杀制者刚性行业可至高位；刃无制者宜把「刚」用在体能与手艺上，不宜投机。',
};

/** 外格职业象（按格外名称或族名查） */
export const WAIGE_CAREER: Record<string, string> = {
  从财格: '为钱做事的格局：金融、贸易、资本运作、理财顾问、大公司商务——依附财气而成，宜进财旺之地与平台，不宜自立门户（日主无根故）。',
  从杀格: '借势上位的格局：大型权力体系内任职（大平台、官府、垄断行业），以服从与执行换位置。宜「大树底下」，忌独立创业。',
  从儿格: '以才华立身：技艺、创作、教育、演艺、咨询——一切靠「输出」吃饭的行当。从儿又生儿（食伤生财）者，才华可直接规模化变现。',
  从强格: '自成一派：学术、宗教、体制深耕、行业协会——在「自己人」的体系里做到极致。忌孤军入财杀之地（陌生商业环境）。',
  专旺格: '把本行做到极致：木宜文化林产、火宜能源光电、土宜地产农业、金宜机械金融、水宜物流贸易——专旺者做本五行的行业天花板最高。',
  化气格: '以化神五行为业：化土宜地产建筑、化金宜机械金融、化水宜流通贸易、化木宜文教、化火宜能源传媒。化气真者，行业即命定。',
  两神成象: '专才领域：气象清纯者在一个窄领域做到顶尖即可，不宜多元铺开。木火通明宜文化传媒、金水相涵宜科研金融、火土成慈宜实业营造。',
};

/** 综合取职业象：正格优先，外格成立（真/存疑）时叠加参考 */
export function careerOf(gejuName: string, waige: { name: string; family: string }[]): string[] {
  const out: string[] = [];
  const top = waige[0];
  if (top) {
    const c = WAIGE_CAREER[top.name] ?? WAIGE_CAREER[top.family];
    if (c) out.push(`${top.name}：${c}`);
  }
  if (GEJU_CAREER[gejuName]) out.push(`${gejuName}：${GEJU_CAREER[gejuName]}`);
  return out;
}
